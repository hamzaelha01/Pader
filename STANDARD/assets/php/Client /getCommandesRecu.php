...
$conn = mysqli_connect("localhost","root","root","Pressing");
	$result = mysqli_query($conn,"SELECT DISTINCT COMMANDE.ID_COMMANDE , COMMANDE.ID_CLIENT , DATE_FORMAT(COMMANDE.DD_COMMANDE, '%Y-%m-%d %H:%i') as DATECOMMANDE  ,COMMANDE.STATUS, DATE_FORMAT(COMMANDE.DF_COMMANDE, '%Y-%m-%d %H:%i') as DATEFCOMMANDE  , SUM(PANIER.QUANITE) as QUANTITE , SUM(PANIER.MONTANT)as MONTANT , LOCALISATION.Adresse_Complete_Collect 
